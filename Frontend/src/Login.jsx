...
function Login() {
  ...
  useEffect(() => {
    ...
    fetch("http://notecraftai-xct5.onrender.com/auth/check", {
      credentials: "include",
    }).then((res) => {
      switch (res.status) {
        case 200:
          ...
          <a href="/auth/google">
            <button className="bg-white px-3 py-2 text-black font-semibold flex flex-row gap-3 h-auto mx-auto items-center text-lg rounded-lg min-w-64">
              ...
              {showButtons ? (
                ...
                <>
                  <div className="border-black mr-auto ml-auto border-l-2 border-b-2 rounded-full h-10 w-10 animate-spin"></div>
                </>
              )}
            </button>
          </a>
